...
export interface StreaksByTeam {
  [team: string]: StreakRecord[];
}

export interface StreaksData {
  winningStreaks: StreaksByTeam;
  losingStreaks: StreaksByTeam;
  hundredPlusStreaks: StreaksByTeam;
  underHundredStreaks: StreaksByTeam;
  aboveFiveHundredStreaks: StreaksByTeam;
  belowFiveHundredStreaks: StreaksByTeam;
}

export interface StreakArrays {
  winningStreaks: StreakRecord[];
  losingStreaks: StreakRecord[];
  hundredPlusStreaks: StreakRecord[];
  underHundredStreaks: StreakRecord[];
}

// Helper to process streaks for each team
export const processTeamStreaks = (teamName: string, matchups: MatchupScoresView[]) => {
...
// Calculate all streaks for all teams
export const calculateStreaks = (matchups: MatchupScoresView[]) => {
  if (!matchups) return {
    winningStreaks: {},
    losingStreaks: {},
    hundredPlusStreaks: {},
  ...
  const allTeams = new Set(
    matchups.flatMap(m => [m.home_team_name, m.away_team_name]).filter(Boolean)
  );
  ...
  allTeams.forEach(team => {
    if (!team) return;
    const teamStreaks = processTeamStreaks(team, matchups);
    
    streaks.winningStreaks[team] = teamStreaks.winningStreaks;
    streaks.losingStreaks[team] = teamStreaks.losingStreaks;
    streaks.hundredPlusStreaks[team] = teamStreaks.hundredPlusStreaks;
    streaks.underHundredStreaks[team] = teamStreaks.underHundredStreaks;
  });

  return streaks;
};

// Convert streak objects to arrays for sorting and display
export const getStreakArrays = (streaks: StreaksData): StreakArrays => {
  const result = {
    winningStreaks: [] as StreakRecord[],
    losingStreaks: [] as StreakRecord[],
    hundredPlusStreaks: [] as StreakRecord[],
...
export const getTop10UniqueTeams = (streakArray: StreakRecord[]) => {
...
export const getAllOtherTeams = (streakArray: StreakRecord[], top10Teams: Set<string>, matchups: MatchupScoresView[]) => {
...
export const formatWeekRange = (start: number, end: number, length: number): string => {
  // For debugging
  if (length > end - start + 1 && end >= start) {
    console.log(`Week range inconsistency: W${start}-W${end} for streak of length ${length}`);
  }
  
  // Make sure weeks are displayed in the correct order (ascending)
