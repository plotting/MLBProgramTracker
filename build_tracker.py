...
.team-pct { margin-left: auto; font-size: 10px; display: flex; flex-direction: column; align-items: flex-end; gap: 1px; }
...
for (const [div, teams] of Object.entries(D.divisions)) {
  ...
  for (const team of teams) {
    ...
    const csMiss = (D.missions[team]['Color Storm'] || []);
    const mjMiss = (D.missions[team]['My Journey']  || []);
    const fK = function(n){ return n >= 1000 ? Math.round(n/1000)+'k' : String(n); };
    const mjXpDone  = mjMiss.reduce(function(s,m){ return s + (m.pct >= 100 ? (parseInt(m.r)||0) : 0); }, 0);
    const mjXpTotal = mjMiss.reduce(function(s,m){ return s + (parseInt(m.r)||0); }, 0);
    const taPct  = mjXpTotal > 0 ? Math.round(mjXpDone / mjXpTotal * 100) : 0;
    const fanMiss = csMiss.filter(function(m){ var k = fanStatKey(m); return k && k !== 'XP'; });
    const fanPct  = fanMiss.length
      ? Math.round(fanMiss.reduce(function(s,m){ return s + m.pct; }, 0) / fanMiss.length)
      : 0;
    let xpLine = '';
    if (taPct < 100 && mjXpTotal > 0) { xpLine = fK(mjXpDone) + '/' + fK(mjXpTotal); }
    const c1       = D.colors[team] ? D.colors[team][0] : '#3b9edd';
    const taColor  = progColor(taPct);
    const fanColor = progColor(fanPct);
    const btn = document.createElement('button');
    btn.className   = 'team-btn';
    btn.dataset.team = team;
    btn.innerHTML =
      '<span class="team-dot" style="background:' + c1 + '"></span>'
      + '<span>' + team + '</span>'
      + '<span class="team-pct">'
      + '<span style="white-space:nowrap">'
      + '<span style="color:' + taColor  + '">' + taPct  + '%</span>'
      + '<span style="color:var(--muted);font-size:9px"> / </span>'
      + '<span style="color:' + fanColor + '">' + fanPct + '%</span>'
      + '</span>'
      + (xpLine ? '<span style="font-size:8px;color:var(--muted);white-space:nowrap">' + xpLine + '</span>' : '')
      + '</span>';
...
const JOLT_POS = {
  // Orioles
  'Adam Jones': 'CF', 'Zack Britton': 'RP', "Darren O'Day": 'RP',
  // Red Sox
  'Dennis Eckersley': 'RP', 'David Ortiz': 'DH', 'Fred Lynn': 'CF',
  // Yankees
  'Derek Jeter': 'SS', 'Graig Nettles': '3B', 'Andy Pettitte': 'SP',
  // Rays
  'Drew Rasmussen': 'SP', 'Zach Eflin': 'SP', 'Fred McGriff': '1B',
  // Blue Jays
  'Roger Clemens': 'SP', 'Tony Fernandez': 'SS', 'José Bautista': 'RF',
  // White Sox
  'Rich "Goose" Gossage': 'RP', 'Larry Doby': 'CF', 'Harold Baines': 'DH',
  // Guardians
  'Jim Thome': '1B', 'Corey Kluber': 'SP', 'Lou Boudreau': 'SS',
  // Tigers
  'Hal Newhouser': 'SP', 'Hank Greenberg': '1B', 'George Kell': '3B',
  // Royals
  'Jeff Suppan': 'SP', 'Mike Moustakas': '3B', 'George Brett': '3B',
  // Twins
  'Johan Santana': 'SP', 'Justin Morneau': '1B', 'Joe Mauer': 'C',
  // Astros
  'Billy Wagner': 'RP', 'Lance Berkman': 'LF', 'Jeff Bagwell': '1B',
  // Mariners
  'Cliff Lee': 'SP', 'Mike Cameron': 'CF', 'Adrian Beltré': '3B',
  // Angels
  'Jered Weaver': 'SP', 'Wally Joyner': '1B', 'Vladimir Guerrero': 'RF',
  // Athletics
  'Vida Blue': 'SP', 'Keith Foulke': 'RP', 'Jason Giambi': '1B',
  // Rangers
  'Gaylord Perry': 'SP', 'Rafael Palmeiro': '1B', 'Ivan Rodriguez': 'C',
  // Braves
  'Greg Maddux': 'SP', 'Terry Pendleton': '3B', 'Andruw Jones': 'CF',
  // Marlins
  'Gary Sheffield': 'RF', 'Al Leiter': 'SP', 'Jeff Conine': 'LF',
  // Mets
  'Tom Seaver': 'SP', 'Mike Piazza': 'C', 'David Wright': '3B',
  // Phillies
  'Chase Utley': '2B', 'Dick Allen': '1B', 'Steve Carlton': 'SP',
  // Nationals
  'Andre Dawson': 'RF', 'Pedro Martinez': 'SP', 'Jeimer Candelario': '3B',
  // Cubs
  'Jake Arrieta': 'SP', 'Ryne Sandberg': '2B', 'Ron Santo': '3B',
  // Reds
  "Paul O'Neill": 'RF', 'Rob Dibble': 'RP', 'Joe Morgan': '2B',
  // Brewers
  'CC Sabathia': 'SP', 'Ted Simmons': 'C', 'Ryan Braun': 'LF',
  // Pirates
  'Roberto Clemente': 'RF', 'Andrew McCutchen': 'CF', 'Bert Blyleven': 'SP',
  // Cardinals
  'Lee Smith': 'RP', 'Red Schoendienst': '2B', 'Matt Carpenter': '3B',
  // Diamondbacks
  'Luis Gonzalez': 'LF', 'Corbin Burnes': 'SP', 'Paul Goldschmidt': '1B',
  // Rockies
  'Ubaldo Jiménez': 'SP', 'Kyle Freeland': 'SP', 'Larry Walker': 'RF',
  // Dodgers
  'Gil Hodges': '1B', 'Eric Gagne': 'RP', 'Manny Ramirez': 'LF',
  // Padres
  'Yu Darvish': 'SP', 'Chase Headley': '3B', 'Mark Davis': 'RP',
  // Giants
  'Chili Davis': 'OF', 'Willie McGee': 'CF', 'Juan Marichal': 'SP',
};
const _JOLT_NAME_PAT = /with (?:Jolt|All-Star|Standout|Breakout|Flashback|Contributor|2nd Half Heroes|Postseason|Rookie|Veteran)\s+(.+?)\s*\./;
  ...
  for (const [team, progs] of Object.entries(D.missions)) {
    ...
    for (const m of (progs['Color Storm'] || [])) {
      ...
      if (total === 500 && !m.t.includes(': Part')) {
        ...
        const dm = (m.d || '').match(_JOLT_NAME_PAT);
        const fullname = dm ? dm[1].trim() : name;
        cs.push({ name, fullname, cur: parseInt(parts[0], 10) || 0, total: 500, pct: m.pct });
      }
    }
    for (const m of (progs['My Journey'] || [])) {
      const parts = m.p.split('/');
      ...
      if (total === 500) {
        ...
        const dm = (m.d || '').match(_JOLT_NAME_PAT);
        const fullname = dm ? dm[1].trim() : name;
        mj.push({ name, fullname, cur: parseInt(parts[0], 10) || 0, total: 500, pct: m.pct });
      }
    }
    result[team] = { cs, mj };
    const all = [...cs, ...mj];
...
function joltCell(card, tag) {
  ...
  const pos = card.fullname ? (JOLT_POS[card.fullname] || '') : '';
  const posBadge = pos ? '<span style="font-size:8px;background:rgba(255,255,255,0.1);color:var(--muted);border-radius:3px;padding:0 3px;margin-left:4px;vertical-align:middle">' + pos + '</span>' : '';
  ...
  const owned = card.fullname ? invMap.get(card.fullname) : null;
  const ownBadge = owned ? '<span title="Card owned" style="font-size:8px;color:#f5c518;margin-left:4px;vertical-align:middle">&#9733;</span>' : '';
  const ownBg = owned ? ';background:linear-gradient(135deg,var(--surface) 0%,rgba(245,197,24,0.07) 100%)' : '';
  if (isDone) {
    return '<td class="ag-td ag-done" style="position:relative' + ownBg + '">'
      + '<div style="font-size:10px;font-weight:600;color:#22c55e">' + card.name + posBadge + tagBadge + ownBadge + '</div>'
  ...
  return '<td class="ag-td' + (isZero ? ' ag-zero' : '') + '" style="position:relative' + ownBg + '">'
    + '<div style="font-size:10px;font-weight:600;color:' + (isZero ? 'var(--muted)' : color) + '">' + card.name + posBadge + tagBadge + ownBadge + '</div>'
